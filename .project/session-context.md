...
6. **Matching por nombre de colecciones reales vs. `dat-index`** — en curso, acumulando datos empíricos (varios sistemas ya probados). Decisión pendiente del algoritmo final: normalizar nombre, matching por hash real vía MAMERedump, o combinación de ambos. A petición del usuario, seguir acumulando datos antes de diseñarlo.
7. **1G1R real de TOSEC** (agrupar variantes regionales, no solo excluir demos/protos/bad dumps) — TOSEC no tiene clonelist ni `cloneof` nativo. Pista sin explorar: ZX-Pokemaster (matching por hash MD5 contra una base tipo ZXDB), sin confirmar si existe equivalente de esa base de hashes para otros microcomputers.
8. **Filtrado por placa/categoría en arcade** (`docs/guides/romsets/workflow.md`, fase de procesado de DAT) — dato ya disponible (`catver.ini` de `antopisa-mame-supportfiles`), nada implementado todavía.
9. **`sistema_index.yaml`** se regenera a mano (`dat-processing reindex`), sin hook automático al tocar una YAML de `dat_sources`.
10. **`rahashes.yaml` sin resolver por sistema** — 5 patrones de ruta sin convención de nombres propia; aparcado a propósito (fichero auxiliar, no prioritario).
11. **Fragmentación residual del 1G1R de Igir en TOSEC** — diagnóstico ya hecho (`tools/scripts/tosec-nonstandard-labels.py`, detecta etiquetas de escena no oficiales por sistema). Falta decidir el mecanismo real de limpieza antes de la pasada de Igir.
12. **Igir**: pasar `--report-output` explícito la próxima vez (el CSV de `igir report` no se localizó en la última prueba); `igir dir2dat` sobre un 1G1R real sin probar todavía.
13. **Normalizador de packs BIOS comunitarios** (idea de tooling, no implementada) — la columna `Hash (MD5)` de `docs/bios.md` ya está rellena y verificada contra fuente oficial para casi toda la tabla de Consolas (solo quedan en `[TODO]` los bootROM de volcado personal sin hash público: `boot9.bin`/`boot11.bin` de 3DS). Falta el script en sí: verificar hash → renombrar al nombre exacto esperado → mover a la subcarpeta correspondiente (`dc/`/`keropi/`/etc.). Sin evaluar todavía.
14. **Alta de `triforce` como sistema arcade** — Dolphin reintegró soporte mainline de la placa Triforce (Namco/Nintendo/Sega: F-Zero AX, Mario Kart Arcade GP, Virtua Striker) el 16/02/2026 (build 2512-395), verificado en fuente oficial. Falta darlo de alta: `docs/systems.md` (Tipo Arcade), `docs/arcade/arcade.md` (core Dolphin) y su fila en `docs/bios.md` (opcional — HLE por defecto, IPL de Triforce + `segaboot.bin` solo para el menú Segaboot completo, mismo patrón que `naomi`/`atomiswave`).
