...
**Sin BIOS confirmado (verificado, no `[TODO]` por omisión):** `atari2600`, `gameandwatch`, `vectrex`, `nes`, `sg1000`, `pcengine` (HuCard, distinto de `pcenginecd` de arriba), `snes`, `jaguar` (Virtual Jaguar incorpora la boot ROM y las BIOS necesarias dentro del propio core, distinto de `jaguarcd`), `n64`, `ngp`, `ngpc`, `wswan`, `wswanc`, `virtualboy`, `xbox360` (Xenia no requiere archivos de sistema de Xbox 360), `psp`/`pspminis`/`psn` (PPSSPP implementa por HLE la BIOS y el sistema operativo de PSP, no utiliza una BIOS real; confirmado también para el catálogo físico UMD), `megaduck` (SameDuck no requiere bootrom). **Sin confirmación primaria explícita:** `supervision` — no se ha localizado todavía una declaración oficial del core seleccionado que confirme expresamente la ausencia de BIOS; no tratar como definitivo si aparece un caso real que lo contradiga.
