...
### Ficheros generados en metadata/dat-index/

Todos gitignored (bajo `metadata/`), regenerables en cualquier momento re-ejecutando el script correspondiente.

- **`<id>.json`** — índice principal: una entrada por familia parent/clone (`name`, `aliases`, `regions`, `languages`, `properties.category`, `members[]`), fuente de la que parten `filter-by-title-type.ps1`, `filter-cross-system-duplicates.ps1`, `find-1g1r-duplicates.ps1`, `inspect-dat-index.ps1` y `generate-romset-docs.ps1`. En `build-dat-index-nointro.ps1`/`.py`: `regions`/`languages` son agregados (unión de todos los clones de la familia); `members[]` da el detalle por-clon individual (`name`, `origin` — `full`/`aftermarket`/`pc` —, `regions`, `languages`, `languageSource`, `category`, `revision`, `version`, `isAlt` propios de ese clon), pensado para un futuro motor de 1G1R por idioma+región (todavía sin implementar el algoritmo de selección en sí — región>idioma>revisión/versión>excluir alt). `revision` es el tag oficial de No-Intro (`(Rev 1)`/`(Rev A)`), `version` la convención de aftermarket/homebrew (`(v1.1)`), `isAlt` marca dumps alternativos (`(Alt)`) a despriorizar/excluir — ambos `null`/`false` si no aplica, no son excluyentes entre sí pero rara vez coinciden en la misma familia. `languages` se resuelve con cadena propia (sin `retool-clonelists-metadata`, ver más abajo): 1) detección por nombre, 2) default por región (solo regiones con idioma dominante claro, sin inventar dato en las ambiguas — `Unknown`/`Europe` sí tienen default: `En`; `Asia` evaluado y descartado por ~26% de error) — `members[].languageSource` (`name`/`region-default`/`none`) deja trazable cuál se usó. La fuente del árbol de familias es seleccionable con `-SourceMode`/`--source-mode` (`FullAftermarket` por defecto, `Pc`, o `Merged` — completa `FullAftermarket` con lo detectado solo en `pc/`); el JSON registra `sourceMode` y qué DAT concreto se usó (`dat`/`datAftermarket`/`datPc`).
- **`aliases/<id>.json`** — capa manual de alias fusionados (auto-detectados vía `cloneofid`/clonelist + los añadidos a mano); se relee y se vuelve a fusionar en cada ejecución, no se sobrescribe sin más.
- **`debug/<id>.json`** — listas de descartados (con motivo: Proto/Demo/Beta/baddump/etc.) y aceptados (nombre completo tal cual en el DAT); solo para revisar patrones y afinar los filtros de descarte.
- **`debug/<id>-pc-contrast.json`** *(solo No-Intro, cuando el sistema tiene pack `pc/`)* — informe de contraste bidireccional entre `pc/` (Parent-Clone) y `full/`+`aftermarket/` combinados: `onlyInPc` (títulos que pc/ tiene y full+aftermarket no) y `onlyInFullAftermarket` (a la inversa). Puramente informativo — `pc/` no participa en la construcción del árbol de familias, solo se usa para detectar huecos de cobertura. Solo se escribe si hay al menos una discrepancia.
- **`debug/<id>-clonelist-diff.json`** *(solo No-Intro, cuando existe clonelist para el sistema)* — divergencias entre la familia resuelta por `cloneofid` y el grupo curado por el clonelist de `retool-clonelists-metadata` (`metadata/dat/retool/clonelists/`), por cada `baseName` presente en ambos. Capa de validación/refuerzo, no sustituye a `cloneofid` en No-Intro (a diferencia de Redump, donde el clonelist sí es la fuente primaria). Solo se escribe si hay al menos una divergencia.
- **`debug/<id>-language-diff.json`** *(solo No-Intro, cuando existe metadata de idioma para el sistema)* — divergencias entre `languages` calculado por la cadena propia (nombre + default por región) y lo curado por `metadata/dat/retool/metadata/<Sistema> (No-Intro).json`, por cada clon individual (más granular que el clonelist-diff, que compara por familia). Retool aquí es **solo validación posterior, opcional** — nunca rellena `languages`. Solo se escribe si hay al menos una divergencia.
