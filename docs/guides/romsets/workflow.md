...
**TOSEC** — descarga del set completo (todas las plataformas en un único paquete) desde **tosecdev.org**; extraer solo el/los DAT del sistema necesario. Útil para microcomputers sin cobertura No-Intro (ej. `amstradcpc`, `sharpx68000`). `gx4000` es la excepción: su set TOSEC es predominantemente homebrew mal mantenido, descartado como fuente (ver `docs/romsets.md`, tratamiento manual pendiente).
