...
| `mame2016`           | MAME `0.174`                   | Último núcleo congelado "MAME por año" de Libretro antes de que solo se mantuviera el `mame` de desarrollo continuo. Fija compatibilidad exacta con sets 0.174 sin el coste de MAME actual. | SBCs de gama media de la era RetroPie (Raspberry Pi 3 y equivalentes) |
...
| :------------------------------ | :----------------------------- | :--------------------------------------------------------------------------------------------------------- | :---------------------------------------------- |
| `AdvanceMame` (1.2 a 5.0)       | MAME `0.106`                   | Monitores CRT antiguos y recreativas comerciales a frecuencias nativas (15kHz). Sin resincronizar con MAME desde la 1.2 — solo importa drivers sueltos versión a versión. | PC (Windows/Linux), Raspberry Pi                |
| `AdvanceMame 0.94`              | MAME `0.94`                    | Build histórica anterior a la congelación en 0.106 — las versiones 0.x de AdvanceMAME seguían el número de versión de MAME al que sincronizaban. | PC (Windows/Linux) antiguos                     |
...
| `GnGeo 0.8.4 (Pandora)`         | GnGeo `0.8.4`                  | Build SDL para Pandora/Pyra con guardado de estado y fix de descifrado de KOF98. [TODO: relación de fork exacta con `GnGeo_2020-02-16` sin confirmar — mismo linaje de GnGeo, versión y rama distintas]. | Pandora, Pyra                                   |
| `PiFBA`                         | FinalBurn Alpha (FBA2X 7.3)    | Puerto SDL original de FBA para Raspberry Pi, predecesor de `fbalpha2012`. Descontinuado, conservado por compatibilidad con romsets históricos. [TODO: versión MAME base sin confirmar en fuente primaria]. | Raspberry Pi 1 (original)                       |
...
| MAME     | MAME 0.174                     | `mame2016`           | —                             | Legacy      |    [ ]     | Último core congelado "MAME por año" antes del `mame` de desarrollo continuo.              |
...
| MAME     | MAME 0.106                     | —                    | `AdvanceMame` (1.2 a 5.0)     | Legacy      |    [ ]     | Diseñado originalmente para salida de vídeo nativa en monitores arcade CRT y TVs. Sin resincronizar con MAME desde la 1.2. |
| MAME     | MAME 0.94                      | —                    | `AdvanceMame 0.94`            | Histórico   |    [ ]     | Build previa a la congelación en 0.106 — el número de versión seguía al de MAME.           |
...
| GnGeo    | GnGeo 0.8.4                    | —                    | `GnGeo 0.8.4 (Pandora)`       | Histórico   |    [ ]     | Build SDL para Pandora/Pyra. [TODO: relación de fork exacta con `GnGeo_2020-02-16` sin confirmar]. |
| FBA      | FinalBurn Alpha (FBA2X 7.3)    | —                    | `PiFBA`                       | Histórico   |    [ ]     | Puerto SDL original de FBA para Raspberry Pi, predecesor de `fbalpha2012`. Descontinuado.  |
